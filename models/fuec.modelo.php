    ...
    static public function mdlDatosFUEC2($datos)
    {
        ...
        if ($tipocontrato == "OCASIONAL"){
            $sql = "SELECT f.*, 
                        ...
                        vm.marca, 
                        tv.tipovehiculo,
                        (SELECT dv.nrodocumento FROM v_re_documentosvehiculos dv WHERE dv.idvehiculo = v.idvehiculo AND dv.idtipodocumento = 3 ORDER BY dv.fechafin DESC, dv.fechacreacion DESC LIMIT 1) AS tarjetaOperacion,
                        oc.objetocontrato, 
